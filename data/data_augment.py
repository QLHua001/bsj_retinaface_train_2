...
def _crop(image, boxes, labels, landm, img_dim, landmark_num):
    ...
    for _ in range(250):
        ...
        centers = (boxes[:, :2] + boxes[:, 2:]) / 2   # 注意， 在这里，boxes的四个值是左上点的坐标和右下点的坐标

        # QTEST 避免半张脸的图片 QTEST QTEST
        mask_a = np.logical_and(roi[:2] < centers, centers < roi[2:]).all(axis=1)
        # mask_a = np.logical_and(roi[:2] < boxes[:, :2], boxes[:, 2:] < roi[2:]).all(axis=1)
        boxes_t = boxes[mask_a].copy()
        labels_t = labels[mask_a].copy()
        landms_t = landm[mask_a].copy()
        landms_t = landms_t.reshape([-1, landmark_num, 2]) # BSJ 检测20个关键点 
...
class preproc(object):
    ...
    def __call__(self, image, targets):
        ...
        image_t = _distort(image_t) # QLHUA TEST 相当于调整图像的像素值
        image_t = _pad_to_square(image_t,self.rgb_means, pad_image_flag) # QLHUA TEST 将图片的宽度用图像均值扩充到跟图像长度一样
